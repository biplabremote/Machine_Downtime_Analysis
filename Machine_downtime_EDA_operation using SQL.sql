...
CREATE TABLE MachineDowntime (
    ...
    `Torque` FLOAT,
    `Cutting` FLOAT,
    `Downtime` VARCHAR(255)
);

-- Load the csv data into SQL
LOAD DATA INFILE '/Dataset/MachineDowntime.csv'
INTO TABLE MachineDowntime
...
SELECT MAX(Hydraulic_Pressure) AS max_hydraulic_pressure
FROM machine_downtime;

SELECT MAX(Coolant_Pressure) AS max_coolant_pressure
FROM machine_downtime;

SELECT MAX(Air_System_Pressure) AS max_air_system_pressure
FROM machine_downtime;

SELECT MAX(Coolant_Temperature) AS max_coolant_temperature
FROM machine_downtime;

SELECT MAX(Hydraulic_Oil_Temperature) AS max_hydraulic_oil_temperature
FROM machine_downtime;

SELECT MAX(Spindle_Bearing_Temperature) AS max_spindle_bearing_temperature
FROM machine_downtime;
...
SELECT MAX(Spindle_Speed) AS max_spindle_speed
FROM machine_downtime;

SELECT MAX(Voltage) AS max_voltage
FROM machine_downtime;

SELECT MAX(Torque) AS max_torque
FROM machine_downtime;

SELECT MAX(Cutting) AS max_cutting
FROM machine_downtime;

-- MIN values for each column
SELECT MIN(Hydraulic_Pressure) AS min_hydraulic_pressure
FROM machine_downtime;

SELECT MIN(Coolant_Pressure) AS min_coolant_pressure
FROM machine_downtime;

SELECT MIN(Air_System_Pressure) AS min_air_system_pressure
FROM machine_downtime;

SELECT MIN(Coolant_Temperature) AS min_coolant_temperature
FROM machine_downtime;

SELECT MIN(Hydraulic_Oil_Temperature) AS min_hydraulic_oil_temperature
FROM machine_downtime;

SELECT MIN(Spindle_Bearing_Temperature) AS min_spindle_bearing_temperature
FROM machine_downtime;
...
SELECT MIN(Spindle_Speed) AS min_spindle_speed
FROM machine_downtime;

SELECT MIN(Voltage) AS min_voltage
FROM machine_downtime;

SELECT MIN(Torque) AS min_torque
FROM machine_downtime;

SELECT MIN(Cutting) AS min_cutting
FROM machine_downtime;
